...
const ghostInfos = [
    ...
    { name : "wraith", evidences : [1,2,7], description: "Wraiths almost never touch the ground, meaning it can't be tracked by footsteps. <span class='weakness'>Wraiths have a toxic reaction to salt.</span>", hunt_threshold: "50%", unique: "After walking through salt and stomping, the wraith will never leave behind green UV footprints. Activity can increase briefly after stepping in a salt pile. Taking a photo of the disturbed salt pile after a wraith steps in it will give no photo rewards. Wraith can teleport silently to a player outside of a ghost event. When it does so, it will leave an EMF reading at the player location then walk back to it's room. After teleporting, Wraith can interact with objects, do ghost events, or even hunt before going back to it's room. Wraith can be found pretty far from it's favorite room. Wraiths cannot walk through walls or teleport during a hunt."},
    ...
    { name : "obake", evidences: [1,3,4], description: "When interacting with the environment, an Obake will rarely leave a trace. <span class='weakness'>Sometimes this ghost will shapeshift, leaving behind unique evidence.</span>", hunt_threshold: "50%", unique: "Obake is a subtle ghost with little to go by. All tells are with it's fingerprints. It is the only ghost that can produce 6 fingered fingerprints in addition to all other normal prints. While fingerprints will always be one of the 2 evidences on NM mode, it still has a 25% chance to not leave one when touching something. Obake can cut the duration of all remaining fingerprints in half if it uses it's ability. If Obake is suspected, pay close attention to FP. Obake has a chance to leave 6 fingered prints."},
...
const last_updated = "10/28/22"
const phasmophobia_server_version = "0.7.1.5"
